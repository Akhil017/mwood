      ...
      <div className="flex min-h-screen w-full flex-col">
        ...
        <main className="flex min-h-[calc(100vh_-_theme(spacing.16))] flex-1 flex-col gap-4 md:gap-8 max-w-4xl w-full mx-auto p-4 lg:p-0">
          <ScrollArea className="h-[80vh]">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 lg:gap-6 mt-6 lg:mt-10">
              {MOODS.map((mood) => (
                <Link
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {/* {MOODS.map((mood) => (
            <Link
              href={`/movie?mood=${mood.mood}`}
              key={mood.mood}
              className="shadow-xs bg-secondary hover:bg-background hover:border-primary duration-300 rounded-xl border"
            >
              <div className="p-4 flex items-center flex-col justify-center gap-2">
                <div className="p-0">
                  <h4 className="text-sm lg:text-lg">{mood.mood}</h4>
                </div>
